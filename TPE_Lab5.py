...
class LabFive():
    ...
    def make_experiment(self):
        self.Y_exp = [[r.randint(math.floor(self.Y_min), math.floor(self.Y_max)) for _ in range(self.m)] for i in range(self.N)]
    ...
    def student_crit(self):
        ...
        for i in range(len(ts)):
            if ts[i] < Stud_teor:
                ...
                self.d += 1
        print("Кількість значимих коефіцієгтів:", self.d)
        print("Коефіціенти після перевірки нуль гіпотези:")
        print(self.b)
